...
export default function ZubarZakaziTermin() {
  ...
  const onInputBrojTelefonaChange = (e) => {

    setPacijent({...pacijent, [e.target.name]: e.target.value });
    setIsDisabled(false);
    loadPacijent(e.target.value);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (isDisabled) {
      await axios.post("http://localhost:8080/zakazi", { trajanje: trajanje, vreme: vremeTermina.value, tippregleda: tipPregleda, brojtelefona: brojtelefona })
    }
    else {
      await axios.post("http://localhost:8080/zakaziNovog", { trajanje: trajanje, vreme: vremeTermina.value, tippregleda: tipPregleda, brojtelefona: brojtelefona, ime: pacijent.ime, prezime: pacijent.prezime, email: pacijent.email })
    }
  ...
  const loadPacijent = async (e) => {
    const result = await axios.get("http://localhost:8080/nadjiPacijenta?brojTelefona=" + e);
              ...
              <div className='col-md-6 offset-md-3 border rounded p-4 mt-2 mb-2 shadow'>
                <form onSubmit={(e) => onSubmit(e)}>
                  <div className='mb-2'>
                    ...
                    <input type={"text"} className='form-control' placeholder="broj telefona" name='brojtelefona' value={brojtelefona} onChange={(e) => onInputBrojTelefonaChange(e)}/>
                  </div>
                  ...
                  <Link className="btn btn-outline-danger mx-2" to="/pregledZubar">Prekini</Link>
                </form>
              </div>
            </div>
          </>
